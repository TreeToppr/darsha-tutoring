...
export default function TutorAvailabilityInline({
    tutorIdProp,
    defaultStartProp,
    defaultEndProp,
    onDefaultsUpdated,
    ...
    useEffect(() => {
        if (!tutorIdProp) return;

        setTutorId(tutorIdProp);

        if (defaultStartProp) setDefaultStart(String(defaultStartProp).slice(0, 5));
        if (defaultEndProp) setDefaultEnd(String(defaultEndProp).slice(0, 5));

        loadBusyWeek(tutorIdProp, weekDays).then(() => setChecking(false));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [tutorIdProp]);
